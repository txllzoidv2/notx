...
if(pc && !reduceMotion){
  var particleCount = isSmallScreen ? 8 : 18;
  var frag = document.createDocumentFragment();
  for(var i=0;i<particleCount;i++){
    var p=document.createElement('div');
    p.className='p';
    p.style.cssText='left:'+Math.random()*100+'%;animation-duration:'+(12+Math.random()*18)+'s;animation-delay:-'+(Math.random()*18)+'s;--dx:'+(Math.random()*50-25)+'px;width:'+(1+Math.random()*1.5)+'px;height:'+(1+Math.random()*1.5)+'px;opacity:.45';
    frag.appendChild(p);
  }
  pc.appendChild(frag);
}
...
(function(){
  'use strict';
  var liquidTargets = '.card,.hero-btn,.nav-login,.video-toggle,.card-cta,.sz,.qty-btn,.btn-buy,.btn-submit,.btn-pp,.btn-logout,.auth-tab,.col-btn,.m-thumb';
  ...
  if(finePointer && !reduceMotion){
    document.addEventListener('pointermove', function(ev){
      ...
      requestAnimationFrame(function(){
        ...
        var el = lastPointerEvent.target.closest('.card,.collection-video-wrap,.modal,.ck-box,.auth-box,.values-inner > *');
        if(el) setPointerVars(el, lastPointerEvent);
      });
    }, {passive:true});
  }
...
(function(){
  ...
  var marqueeWidth = loopWidth();
  window.addEventListener('resize', function(){ marqueeWidth = loopWidth(); }, {passive:true});
  ...
  function frame(now){
    ...
    x -= speed * dt;
    if(marqueeWidth && Math.abs(x) >= marqueeWidth) x += marqueeWidth;
